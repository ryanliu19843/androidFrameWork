/*
 * 文件名: Verify.java 版 权： Copyright XCDS Tech. Co. Ltd. All Rights Reserved. 描 述:
 * [该类的简要描述] 创建人: ryan 创建时间:2014-1-7
 * 
 * 修改人： 修改时间: 修改内容：[修改内容]
 */
...
public class Verify {
	...
	public static boolean isMobile(String phone) {
		...
		Pattern pattern = Pattern.compile("^[1]([0-9]{1}[0-9]{1})[0-9]{8}$");
	...
	public static Verify validateIdCard(String IDStr) {
		...
		if ((IDStr.length() != 15) && (IDStr.length() != 18)) {
			errorInfo = Frame.CONTEXT.getString(R.string.verify_carid_num);
		...
		if (!isNumeric(Ai)) {
			errorInfo = Frame.CONTEXT.getString(R.string.verify_carid_lasnum);
			return new Verify(false, "", errorInfo);
		}
		...
		if (!isDate(strYear + "-" + strMonth + "-" + strDay, "yyyy-MM-dd")) {
			errorInfo = Frame.CONTEXT.getString(R.string.verify_carid_birthday);
			return new Verify(false, "", errorInfo);
		}
		...
		if ((Integer.parseInt(strMonth) > 12) || (Integer.parseInt(strMonth) == 0)) {
			errorInfo = Frame.CONTEXT.getString(R.string.verify_carid_montherror);// "身份证月份无效";
			return new Verify(false, "", errorInfo);
		}
		if ((Integer.parseInt(strDay) > 31) || (Integer.parseInt(strDay) == 0)) {
			errorInfo = Frame.CONTEXT.getString(R.string.verify_carid_dayerror);// "身份证日期无效";
			return new Verify(false, "", errorInfo);
		}
		Hashtable<String, String> h = GetAreaCode();
		String address = h.get(Ai.substring(0, 2));
		if (address == null) {
			errorInfo = Frame.CONTEXT.getString(R.string.verify_carid_addrerror);// "身份证地区编码错误。";
			return new Verify(false, "", errorInfo);
		}
		int TotalmulAiWi = 0;
		for (int i = 0; i < 17; i++) {
		...
		if (IDStr.length() == 18) {
			if (!Ai.equals(IDStr)) {
				errorInfo = Frame.CONTEXT.getString(R.string.verify_carid_error);// "身份证无效，不是合法的身份证号码";
				return new Verify(false, "", errorInfo);
			}
		} else {
	...
	private static Hashtable<String, String> GetAreaCode() {
		...
		hashtable.put("11", Frame.CONTEXT.getString(R.string.verify_carid_addres_11));
		hashtable.put("12", Frame.CONTEXT.getString(R.string.verify_carid_addres_12));
		hashtable.put("13", Frame.CONTEXT.getString(R.string.verify_carid_addres_13));
		hashtable.put("14", Frame.CONTEXT.getString(R.string.verify_carid_addres_14));
		hashtable.put("15", Frame.CONTEXT.getString(R.string.verify_carid_addres_15));
		hashtable.put("21", Frame.CONTEXT.getString(R.string.verify_carid_addres_21));
		hashtable.put("22", Frame.CONTEXT.getString(R.string.verify_carid_addres_22));
		hashtable.put("23", Frame.CONTEXT.getString(R.string.verify_carid_addres_23));
