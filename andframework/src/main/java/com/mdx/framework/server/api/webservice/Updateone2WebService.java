...
public class Updateone2WebService implements ApiRead {
    ...
    private void setFileMd5(UpdateOne updateone) {
        ...
        if (updateone.getPostdata() != null && updateone.getPostdata() instanceof ArrayList) {
            ...
            try {
                ...
                updateone.setMd5str(Md5.md5(bos.toByteArray()) + "_" + updateone.getPage() + "_"
                        + updateone.getPageSize());
            } catch (Exception e) {
    ...
    public Son readSon(UpdateOne updateone) {
        ...
        if (updateone.isSaveAble()) {
            byte[] bytes = DataStoreCacheManage.readByte(updateone.getMd5str(),
                    network == 0 ? System.currentTimeMillis() : updateone.getCacheTime());
            if (bytes != null) {
                synchronized (this) {
                    cacheson = new Son2WebService(new String(bytes), updateone);
        ...
        IntenetRead2WebService ir = new IntenetRead2WebService();
        updateone.setServerIntermit(ir);
        try {
            String value = ir.get(ParamsManager.get(ParamsManager.PARAM_WEBSERVICE_NAMESPACE),
                    ParamsManager.get(ParamsManager.PARAM_WEBSERVICE_URL),
                    updateone.getUrl(),
                    params);
            synchronized (this) {
                son = new Son2WebService(value, updateone);
    ...
    public Son readBuild(UpdateOne updateone) {
        ...
        return new Son2WebService(new String(bytes), updateone);
    }
