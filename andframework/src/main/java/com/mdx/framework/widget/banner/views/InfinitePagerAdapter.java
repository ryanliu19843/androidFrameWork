package com.mdx.framework.widget.banner.views;
...
public class InfinitePagerAdapter extends PagerAdapter {
    ...
    public InfinitePagerAdapter(MAdapter adapter) {
        this.adapter = adapter;
    }

    public InfinitePagerAdapter(CardAdapter adapter) {
    ...
    public void finishUpdate(ViewGroup container) {
        ViewPager viewPagerEx= (ViewPager) container;
        int curritem=viewPagerEx.getCurrentItem();
        int relcount=getRealCount();
        if(relcount==0){
            return;
        }
        if(curritem<relcount*2 || curritem>relcount*3){
            viewPagerEx.setCurrentItem(curritem%relcount+relcount*2,false);
        }
    }
