...
final class CameraConfigurationManager {
    ...
    public int marginleft = 0, margintop = 0;
    ...
    void initFromCameraParameters(Camera camera, SurfaceHolder holder, View view) {
        ...
        int w = cameraResolution.y, h = cameraResolution.x, rw = 0, rh = 0, ml = 0, mt = 0;
        float fw = (w * 1f) / (rect.width() * 1f), fh = (h * 1f) / (rect.height() * 1f);
        if (fw != fh) {
            if (fw < fh) {
                rh = rect.height();
                rw = (int) (w / fh);
            ...
            RelativeLayout.LayoutParams layout = new RelativeLayout.LayoutParams(rw, rh);
            layout.setMargins(-ml, -mt, 0, 0);
            marginleft = -ml;
            margintop = -mt;
