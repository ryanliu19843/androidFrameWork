...
public final class GifInfoHandle {
    ...
    synchronized long renderFrame(Bitmap frameBuffer, int blur) {
        long retn = renderFrame(gifInfoPtr, frameBuffer);
        if (blur > 0) {
            Util.fastblur(Frame.CONTEXT, frameBuffer, blur, false);
        } else if (blur < 0) {
            Util.fastblur(Frame.CONTEXT, frameBuffer, -blur, false);
        }
        return retn;
    ...
    synchronized void seekToTime(@IntRange(from = 0, to = Integer.MAX_VALUE) final int position, final Bitmap buffer, int blur) {
        seekToTime(gifInfoPtr, position, buffer);
        if (blur > 0) {
            Util.fastblur(Frame.CONTEXT, buffer, blur, false);
        } else if (blur < 0) {
            Util.fastblur(Frame.CONTEXT, buffer, -blur, false);
        }
    }
