...
public class GifDrawable extends Drawable implements Animatable, MediaPlayerControl {
    ...
    public GifDrawable() {
        ...
        mIsRenderingTriggeredOnDraw = false;
     ...
     * This method is thread-safe.
     */
    public void reset(final boolean autostart) {
    ...
    public void seekTo(@IntRange(from = 0, to = Integer.MAX_VALUE) final int position) {
        ...
        mExecutor.execute(new SafeRunnable(this) {
            @Override
            public void doWork() {
                mNativeInfoHandle.seekToTime(position, mBuffer, blur);
    ...
    public void draw(@NonNull Canvas canvas, Rect mDstRect, Paint paint) {
        final boolean clearColorFilter;
